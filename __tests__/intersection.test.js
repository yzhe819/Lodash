...
it('should return an array of unique values', () => {
    expect(
        intersection([1, 1, 3, 2, 2], [5, 2, 2, 1, 4], [2, 1, 1])
    ).toStrictEqual([1, 2]);
});
...
it('should work with `arguments` objects', () => {
    expect(intersection([0, 1, null, 3], toArgs([1, 2, 3]))).toStrictEqual([
        1, 3,
    ]);
    expect(intersection(toArgs([1, 2, 3]), [0, 1, null, 3])).toStrictEqual([
        1, 3,
    ]);
});
